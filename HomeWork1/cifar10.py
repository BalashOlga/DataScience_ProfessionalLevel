...
PROJECT_FOLDERS = ['models', 'experiments', 'database', 'Experiments']
...
# Функция для сохранения артефактов эксперимента
def save_experiment_artifacts(experiment_name, plots, metrics_df, model_type, parameters):
    """Сохраняет графики и артефакты эксперимента в папку Experiments"""
    try:
        ...
        for plot_name, plot in plots.items():
            filename = f"{exp_folder}/{plot_name}_{experiment_name}.png"
            plot.savefig(filename, dpi=300, bbox_inches='tight')
            plt.close(plot)  # Закрываем график чтобы освободить память
        
        # Сохраняем метрики в CSV
        metrics_filename = f"{exp_folder}/metrics_{experiment_name}.csv"
        metrics_df.to_csv(metrics_filename, index=True)
        
        # Сохраняем параметры эксперимента в JSON
        params_filename = f"{exp_folder}/parameters_{experiment_name}.json"
        with open(params_filename, 'w', encoding='utf-8') as f:
            json.dump(parameters, f, ensure_ascii=False, indent=2)
        
        # Создаем README файл с информацией об эксперименте
        readme_content = f"""
...
- Параметры: см. parameters_{experiment_name}.json

## Сохраненные файлы
- Графики: {len(plots)} файлов PNG
- Метрики: metrics_{experiment_name}.csv
- Параметры: parameters_{experiment_name}.json

## Метрики модели
{metrics_df.to_string()}
"""
        with open(f"{exp_folder}/README.md", 'w', encoding='utf-8') as f:
            f.write(readme_content)
        
        return True, f"Артефакты сохранены в {exp_folder}"
    
    except Exception as e:
        return False, f"Ошибка сохранения артефактов: {str(e)}"

# Функция для сохранения графиков обучения
def save_training_plots(history, experiment_name, model_type):
    """Сохраняет графики обучения нейронной сети"""
    try:
        exp_folder = f"Experiments/{experiment_name}"
        os.makedirs(exp_folder, exist_ok=True)
        ...
        fig1.savefig(f"{exp_folder}/training_accuracy_{experiment_name}.png", dpi=300, bbox_inches='tight')
        plt.close(fig1)
        ...
        fig2.savefig(f"{exp_folder}/training_loss_{experiment_name}.png", dpi=300, bbox_inches='tight')
        plt.close(fig2)
        
        return True
    except Exception as e:
        st.error(f"Ошибка сохранения графиков обучения: {e}")
        return False
...
def create_extended_plots(y_true, y_pred, y_pred_proba=None, class_names=None):
    ...
    if y_pred_proba is not None:
        ...
        ax4.set_xlabel('Уверенность предсказания')
        ax4.set_ylabel('Количество примеров')
        ax4.set_title('Распределение уверенности предсказаний')
        ax4.grid(True, alpha=0.3)
        plots['confidence_distribution'] = fig4
...
def check_project_structure():
    ...
    folders = ['models', 'experiments', 'database', 'Experiments']
    status = {}
...
# Раздел 2: K-Nearest Neighbors (простая модель)
    ...
    if st.button("Обучить K-NN модель"):
        with st.spinner('Обучаем K-NN... это может занять несколько минут'):
            ...
            plots, metrics_df = create_extended_plots(y_true, y_pred, None, class_names)
            
            # Показываем графики
            st.subheader("📈 Анализ производительности модели")
            for plot_name, plot in plots.items():
                st.pyplot(plot)
            ...
            # Сохраняем артефакты эксперимента
            parameters = {
                'k_value': k_value,
                'algorithm': 'auto',
                'weights': 'uniform',
            ...
            artifact_success, artifact_message = save_experiment_artifacts(
                experiment_name, plots, metrics_df, 'K-NN', parameters
            )
            
            if artifact_success:
                st.success(f"📊 {artifact_message}")
            else:
                st.warning(f"⚠️ {artifact_message}")
...
# Раздел 3: Простая нейронная сеть
    ...
    if st.button("Обучить нейронную сеть"):
        with st.spinner('Обучаем нейронную сеть...'):
            ...
            save_training_plots(history, experiment_name, 'Neural Network')
            ...
            artifact_success, artifact_message = save_experiment_artifacts(
                experiment_name, plots, metrics_df, 'Neural Network', parameters
            )
...
# Раздел 4: Свёрточная нейронная сеть (CNN)
    ...
    if st.button("Обучить CNN"):
        with st.spinner('Обучаем свёрточную сеть... это займёт некоторое время'):
            ...
            # Сохраняем графики обучения
            save_training_plots(history, experiment_name, 'CNN')
            ...
            artifact_success, artifact_message = save_experiment_artifacts(
                experiment_name, plots, metrics_df, 'CNN', parameters
            )
...
# Раздел 6: Сравнение экспериментов
    ...
    if experiments_df.empty:
        ...
        # 7. Просмотр файлов экспериментов
        st.subheader("📁 Файлы экспериментов")
        
        if os.path.exists('Experiments'):
            experiments_folders = [f for f in os.listdir('Experiments') if os.path.isdir(os.path.join('Experiments', f))]
            
            if experiments_folders:
                selected_exp_folder = st.selectbox("Выберите эксперимент для просмотра файлов:", experiments_folders)
                
                if selected_exp_folder:
                    exp_path = f"Experiments/{selected_exp_folder}"
                    files = os.listdir(exp_path)
                    
                    st.write(f"**Файлы в эксперименте '{selected_exp_folder}':**")
                    for file in sorted(files):
                        ...
                        with col3:
                            if file.endswith('.png'):
                                if st.button("👁️", key=f"view_{file}"):
                                    st.image(file_path)
                ...
                st.info("Папка Experiments пуста")
        else:
            st.warning("Папка Experiments не существует")
